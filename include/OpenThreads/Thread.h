...
public:
	...
	/** Waits on the cancel event until the timeout specified
	  * in microseconds. If the cancel event is set, testCancel() is
	  * called.
	  * Win32 only. On other implementations this is equivalent
	  * to microSleep() (FIXME).
	  */
	...
	/** getTickCount(), returns the millisecs since boot
	  * (Not strictly thread API, see remark in microSleep()
	*/
